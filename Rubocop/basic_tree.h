...
//  Rubocop
//
//  Created by doob on 2015-05-15.
//  Copyright (c) 2015 Jacob Carlborg. All rights reserved.
//

#ifndef Rubocop_basic_tree_h
#define Rubocop_basic_tree_h
...
namespace oak
{
	...
	public:
		struct iterator : std::iterator<std::bidirectional_iterator_tag, value_type>
		{
			...
			value_type& operator*  ()             { return _info;  }
			value_type* operator-> ()             { return &_info; }
			value_type const& operator*  () const { return _info;  }
			value_type const* operator-> () const { return &_info; }
